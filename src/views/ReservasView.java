...
public class ReservasView extends JFrame {
	...
	private JLabel labelAtras;
        public static int valorXDia=28;        
	/**
	...
	public ReservasView() {
		...
		lblCheckIn.setBounds(68, 136, 369, 14);
		...
		lblCheckOut.setBounds(68, 221,369, 14);
...
//Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
		if(txtFechaE.getDate()== null){
                    txtFechaE.getDateEditor().getUiComponent().requestFocusInWindow();
                }else{
                    Calendar fechaIngreso = txtFechaE.getCalendar();
                    Calendar fechaEgreso = txtFechaS.getCalendar();
                    double diasEstadia = 0; 
                    
                    if (fechaEgreso.compareTo(fechaIngreso)<=0){
                        JOptionPane.showMessageDialog(null, "La Fecha de Salida debe ser posterior a la Fecha de Entrada.");
                        txtFechaS.getDateEditor().getUiComponent().requestFocusInWindow();
                    }else{
                        //calculo días de estadía
                        diasEstadia = Math.abs(fechaEgreso.getTimeInMillis() - fechaIngreso.getTimeInMillis())/ (1000*24*3600);
                        //ahora calculamos monto a pagar
                        double montoEstadia = diasEstadia * valorXDia;
                        txtValor.setText(String.valueOf(montoEstadia));
                    }
		...
		lblValor.setBounds(72, 303, 369, 14);
