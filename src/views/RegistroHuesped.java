...
public class RegistroHuesped extends JFrame {
	 ...
	 * Create the frame.
	 */
	public RegistroHuesped() throws SQLException {
		...
		contentPane.add(lblNumeroReserva);
                ...
                //vamos a obtener último número de reserva en bbdd
                reservasController nroReserva = new reservasController();
                int numero = nroReserva.getNumeroReserva()+1;
		txtNreserva = new JTextField();
                txtNreserva.setText(String.valueOf(numero));
		...
		btnguardar.addMouseListener(new MouseAdapter() {
			...
			if (!nombre.equalsIgnoreCase("")&& !apellido.equalsIgnoreCase("") &&
                                !nacimiento.equalsIgnoreCase("yyyy-MM-dd") && !telefono.equalsIgnoreCase("")){
                            //perdurar en BBDD la reserva
                        
                            reservasController controller= new reservasController();
                            String fechaEntrada = ((JTextField)ReservasView.txtFechaE.getDateEditor().getUiComponent()).getText();
                            String fechaSalida = ((JTextField)ReservasView.txtFechaS.getDateEditor().getUiComponent()).getText();
                            String valor = ReservasView.txtValor.getText();
                            int fPago = ReservasView.txtFormaPago.getSelectedIndex()+1;
                                try {
                                    int resultado = controller.guardarReserva(fechaEntrada,fechaSalida,valor,fPago);

                                    if (resultado != 0){
                                        huespedesController controllerH= new huespedesController();
                                        controllerH.guardarHuesped(nombre,apellido, nacimiento,nacionalidad, telefono,resultado);
                                        txtNombre.setText("");
                                        txtApellido.setText("");
                                        ...
                                        JOptionPane.showMessageDialog(null, "La Reserva ha sido guardada exitosamente. \n Nro de Reserva: "+resultado);
                                    }else{
                                        throw new SQLException();
                                    }
                                } catch (SQLException ex) {
                                    JOptionPane.showMessageDialog(null, "Ha ocurrido un error la reserva no ha sido guardada, por favor reintente");
                                }       
                        
                        
                        }else{
                         JOptionPane.showMessageDialog(null, "Ningún campo pueden estar vacios");
                        }
